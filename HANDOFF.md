...
| エージェントが tangle を理解しているか | 未検証。§2.22 の C は散文だけの節だったので問われていない。**コードを変えざるを得ない依頼**（例: 却下された行に取り消し線）で試す |
...
**前半は完了（§2.9）。** 置けることと、置いても App が壊れないことは確認済み。

**A（一覧）と B（読解）も完了（§2.22）。** エージェントは `.nw` を認識し、
533 行の文書を 3,332 文字で正確に要約した。プラットフォームがファイル一覧を
システムプロンプトで渡すので、**未知の拡張子でも見落とされない**。

**C（編集）は半分。** エージェントは `server.js` ではなく `.nw` を編集した——
つまり**予期した反証は起きなかった**。ただし追加されたのが散文だけの節だったため、
**tangle は問われていない**。

#### 残っているのはこれ

**コードを変えざるを得ない依頼で、tangle の理解を試す。**

> 却下された行に取り消し線を引くようにして、変更を反映して

これなら `<<記録を描き直す>>` チャンクを直す必要があり、`client.js` を
再生成しないと画面が変わらない。観察したいのは次の分岐である。

- `.nw` のチャンクを直し、`client.js` も辻褄を合わせる → **tangle を理解している**
- `.nw` だけ直して `client.js` を放置 → 文書は正しいが**動かない**
- `client.js` だけ直して `.nw` を放置 → 動くが**原本が嘘になる**

三つ目が起きたら、それは**仮説にとって重要な反証**である。
「散文が一次でコードが二次」という構成が、エージェント側から見て
自然でないということだからである。

**費用の目安**（§2.22 の実測）。編集させる往復は 5,000〜6,000 neurons。
無料枠 10,000/日なので **1 日 1〜2 回**が現実的な線になる。
