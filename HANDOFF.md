...
- **エージェントは `.nw` を読むか。** 拡張子を知らないファイルをどう扱うか。
  散文とコードの混在を理解するか
- **tangle をどこで走らせるか。** これは §3 に「未決」として残っている論点で、
  Gadget にはビルド工程がない。エージェントが自分で展開して書き戻すのか、
  それとも `.nw` を放置して `server.js` を直接いじるのか

後者が起きたら、それは**仮説にとって重要な反証**になる。
