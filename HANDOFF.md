...
| 無料枠がいつ戻るか | **未検証。UTC 日付ではない**。8/15 15:26 に使い切り、8/16 07:22（当日消費 0）でもまだ 429 だった。移動 24 時間の窓と推測しているが確かめていない（§2.25） |
| neuron 換算が正しいか | 未検証。`$0.011/1000` で線形と仮定した推定値であり、Cloudflare の実カウントとは違うかもしれない。ダッシュボードの実測と突き合わせていない |
